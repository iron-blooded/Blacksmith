...
import java.util.*;
public class applyingEnchantment implements CommandExecutor{
    private final Blacksmith plugin;
    public applyingEnchantment(Blacksmith plugin) {
        this.plugin = plugin;
    }
    @Override
            ...
            for (Enchantment ench : matchingEnchants.keySet()) {
                if (ench.getKey().getKey().equals(args[0])){
                    ...
                    if (lvl_blacksmith >= 0  || lvl_blacksmith == -2) {
                        plugin.transferEnchantment(ench, item_in_off_hand, item_in_main_hand, 0);
//                        if (item_in_off_hand.getEnchantments().size() == 0
//                                && item_in_off_hand.getItemMeta().getDisplayName().equals(ench.getName() + " " + matchingEnchants.get(ench))){
//                            ItemMeta meta = item_in_off_hand.getItemMeta();
//                            meta.setDisplayName("");
//                            item_in_off_hand.setItemMeta(meta);
//                        }
                        sender.sendMessage("§aВы успешно перенесли зачарования!");
                    }
                    else {
